...
def error(array):  
        ...
        return math.sqrt(np.mean((array[50:950, 50:950])**2))
...
def clip(array):
    tmp = np.array(array.astype(float))
    tmp = tmp - BIAS
    #print(tmp)
...
def opt(sum, images, offsets, out_fn):
        ...
        for iter in range(52000000):
                ...
                old_sum = np.copy(sum)                          #save the current sum of images
                ...
                if (iter % 130 == 0):
                        cv2.imshow('sum', ((1.0/sliders[1]) * ((sum/3550.0) - sliders[0])))
                        cv2.imshow('chunk', ((1.0/sliders[1]) * (chunk((sum/3550.0)) - sliders[0])))
                        if cv2.waitKey(1) == 27:
                                break
                ...
                if (iter % 20000 == 0):                         #every N frames, recalc the whole stack to avoid
                        ...
                        fits.writeto(out_fn, sum, hdr, overwrite=True)
